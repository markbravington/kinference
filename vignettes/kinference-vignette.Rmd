...
alleles and so on; see MS^[Which doesn't exist _yet_...]. The function for this is `hetzminoo_fancy`, which has a "rich" variant optimized for finding contamination, and a "poor" variant for degradation. With "rich", you're looking for too-big values; with "poor", for too-small values--- although it must be said that the two variants tend to look pretty similar.


```r
hetzpoor <- hetzminoo_fancy(bluefin_3, "poor") ## a couple of outlying low samples here. 
```

![plot of chunk bluefinHetzes](./figures-kinference/bluefinHetzes-1.png)

```r
hetzrich <- hetzminoo_fancy(bluefin_3, "rich") ## no high ones
```
